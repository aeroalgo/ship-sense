...
```python
# BAD: Global instance at module import time
class JobService:
    def __init__(self, storage=None, bus=None, dispatcher=None):
        self.storage = storage or S3Storage()
        self.bus = bus or InMemoryEventBus()
        self.dispatcher = dispatcher or JobTaskDispatcher()

job_service = JobService()  # Created on import — real S3, Celery, etc.
```

**Why it's bad:**
- Side effects at import time (real clients connect before app startup)
- Cannot inject fakes in tests without monkeypatching modules
- Violates FastAPI DI (`Depends`) and project pattern: Depends → Service → Repository
- No explicit lifecycle (startup/shutdown in `lifespan`)

**Fix:** Factory in `deps.py` + `Depends` in endpoints.

```python
# GOOD: app/jobs/deps.py
...
# GOOD: endpoint
@router.post("/jobs")
async def create_job(service: JobServiceDep, db: AsyncSession = Depends(get_db)):
    return await service.create_job(db, ...)
```

```python
# GOOD: tests — dependency_overrides, no global patching
app.dependency_overrides[get_job_service] = lambda: JobService(
    storage=fake_storage, bus=fake_bus, dispatcher=fake_dispatcher
)
```

**Rule:** Do not add `*_service = *Service()` at the bottom of domain `service.py` files.
...
- [ ] No module-level service singletons (`foo_service = FooService()`)
...
| Module-level service singleton | `get_*_service()` in `deps.py` + `Depends` |
